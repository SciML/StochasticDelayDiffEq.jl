...
function DiffEqBase.__init(
  ...
  adaptive = StochasticDiffEq.isadaptive(alg),
  gamma=9//10,
  abstol=nothing,
  reltol=nothing,
  qmax=StochasticDiffEq.qmax_default(alg.alg), qmin=StochasticDiffEq.qmin_default(alg.alg),
  ...
  seed = UInt64(0), alias_u0=false, # keyword arguments for DDEs
  ...
  if !alg_compatible(prob,alg.alg)
  ...
  @unpack f, g, p, neutral, constant_lags, dependent_lags = prob
  ...
  #rate_prototype = rate_prototype_of(u0, tspan) TODO: in DDE this line is used
  ...
  #####################################
  ####### TODO History Wrapping #######
  #####################################
  history = build_history_function(prob, alg, rate_prototype, reltol_internal; dt = dt, adaptive = adaptive, internalnorm = internalnorm)
  ...
  # TODO: maximum_order (maximum strong order?)
  maximum_order = 1.5; # form the SDE solvers' documentation page

  tstops_internal, saveat_internal, d_discontinuities_internal =
    DelayDiffEq.tstop_saveat_disc_handling(tstops, saveat, d_discontinuities, tspan,
  ...
  solution_arrays = recursivecopy(u)
  ...
  cache = StochasticDiffEq.alg_cache(alg.alg,prob,u,W.dW,W.dZ,p,rate_prototype,noise_rate_prototype,uEltypeNoUnits,uBottomEltypeNoUnits,tTypeNoUnits,uprev,f_with_history,t,dt,Val{isinplace(prob)})
