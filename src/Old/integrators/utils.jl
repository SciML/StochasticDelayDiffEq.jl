"""
    advance_sde_integrator!(integrator::SDDEIntegrator)

Advance the SDE integrator of `integrator` to the next time interval by updating its values
and interpolation data with the current values and a full set of interpolation data of
`integrator`.
"""
function advance_sde_integrator!(integrator::SDDEIntegrator)
  @unpack f, u, t, p, k, dt, uprev, alg, cache = integrator
  ode_integrator = integrator.integrator

  # algorithm only works if current time of SDDE integrator equals final time point
  # of solution
  t != ode_integrator.sol.t[end] && error("cannot advance ODE integrator")

  # complete interpolation data of SDDE integrator for time interval [t, t+dt]
  # and copy it to ODE integrator
  # has to be done before updates to SDE integrator, otherwise history function
  # is incorrect
  if iscomposite(alg)
