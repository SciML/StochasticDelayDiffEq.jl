function DiffEqBase.__init(prob::DiffEqBase.AbstractDDEProblem,
  ...
  # abstol_internal = get_abstol(u0, tspan, alg.alg; abstol = abstol) # TODO same, but prettier than in ODE
  # reltol_internal = get_reltol(u0, tspan, alg.alg; reltol = reltol) # TODO same, but prettier than in ODE
  ...
  destats = DiffEqBase.DEStats(0) # TODO ???
  ...
  # reserve capacity for the solution # TODO: from DelayDiffEq
  ...
  integrator = DDEIntegrator{typeof(alg.alg),..,typeof(u0),..,..,..,..,..,..,typeof(sol),
                             ...
                             OrdinaryDiffEq.fsal_typeof(alg.alg.., ..),
                             ..,..}(
                             .., ..,.., t0,.., f_with_history, ..,
                             ..,..,.., prev_idx, prev2_idx,
                             _fpsolver,order_discontinuity_t0, tracked_discontinuities, discontinuity_interp_points, discontinuity_abstol, iscontinuity_reltol, alg.alg,
                               ..,..,..,..,..,..,..,..,..,..,..,.., length(ts), length(ts),..,..,..,..,..,..,..,..,
                               ..,..,..,..,..,..,.., history, ode_integrator)
...
# TODO: basically same as for the ODE!!!!!
function DiffEqBase.solve!(integrator::DDEIntegrator)
  @unpack tdir, opts, sol = integrator # TODO prettier than in ODE
  @unpack tstops = opts # TODO prettier than in ODE
...
function OrdinaryDiffEq.initialize_callbacks!(integrator::DDEIntegrator,
  ...
  if u_modified
    # TODO ...
    # TODO these are added compared to the ODE
...
function OrdinaryDiffEq.tstop_saveat_disc_handling(tstops, saveat, d_discontinuities, tspan,order_discontinuity_t0, alg_maximum_order, constant_lags,neutral)
